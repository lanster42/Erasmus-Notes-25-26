...
\begin{enumerate}[label= (\alph*)]
 ...
 \item \includegraphics[scale=0.107]{2ex_a}
 ...
 \item If I understood the question correctly then $x$ is an element of ${2, 3, \ldots 556}$. We removed 0 and 1 because $0^{k} = 0 \neq 321 (\mod n)$ and $1^{k} = 1 \neq 321 (\mod n)$ for $n > 1$ which is our case.
 \item while looking through youtube to find a good explanation for why we can use extended euclidean algorithm for finding the inverse, I found that this one explains it well. I used it to solve this problem: \href{https://www.youtube.com/watch?v=IwRtISxAHY4}{link}
 \includegraphics[scale=0.11]{2ex_d}

 The result was: $-197 \equiv 556 - 197 \equiv 359 (\mod 556)$

 \textit{Observation}: This method looks promising for implementation into python but too error prone for hand calculations.
 \item Please correct me if I'm wrong, but because 557 is prime, we can translate our problem into a cyclic group problem where we look at group $mathbb{Z}_{p}^*$. Here we also learnt that $ p \in \mathbb{P} \implies \mathbb{Z}_{p}^* \cong (\mathbb{Z}_{p - 1}, +)$. So when the base is in mod 557, we can regard the exponent as being in mod 556. So for that reason ${(x^{127})}^{357} = {{(x^{127})}^{127}}^{-1} = x^{127 * {127}^{-1}} = x^1$.
 
 From your instructions I speculated that we can maybe generalize this notion for when 557 is not prime. So for $m \in \mathbb{N}$ we could generally say that $\mathbb{Z}_{m}^* \equiv (\mathbb{Z}_{\phi(m)}, +)$. I proposed this question to duck.ai which said that we cannot generally speculate this because the structures are generally different: $\mathbb{Z}_{m}^*$ is not generally cyclic and $(\mathbb{Z}_{n}, +)$ is always cyclic for $n \in \mathbb{N}$. They do, however, have the same cardinality (although I don't know is this could be useful).

 \item I answered this question with (e). The final solution to the congruence is then of course: $321^{t} = 321^{359}$.
 \item since I didn't really understand the way we did the square-and-multiply algorithm in class, I once again found a short explanation video which I used as a reference: \href{https://www.youtube.com/watch?v=8r4-5k-o1QE}{link}
 Here are my computations:

\includegraphics[scale=0.12]{2ex_g}

If my computations are correct (from which I'm sure there's a large probability of human error), the solution is: $321^{359} \mod 557 = 106$.

When the oportunity arises, I will look into why the square-and-multiply method works. For now I will be okay with the result.
\end{enumerate}
